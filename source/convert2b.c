...
void gen_out_prefix(char* line) {
  ...
  for (str = mline; ; str = NULL) {
    ...
    if (0 == strncmp(token,"20", 2)) {
      ...
      if (i == 1) {
        sprintf(out_prefix1 + strlen(out_prefix1), "_%s", token);
        break;
      }
      i ++;
    }
...
int convert(char** files, int files_cnt, char* output_dir, char** shift_files, int shift_files_cnt) {
  // 把2B文件按内容的时间排序
  struct sort_t * arr = malloc(sizeof(struct sort_t) * files_cnt);
  memset(arr, 0, sizeof(struct sort_t)* files_cnt);
  int i = 0;
  for(i = 0; i < files_cnt; i ++) {
    snprintf(arr[i].filename, 499, "%s", files[i]);
    arr[i].time = read_first_time_2B(files[i]);
  }
  qsort(arr, files_cnt, sizeof(struct sort_t), cmpfunc);

  // 把shift文件按内容的时间排序
  if (shift_files != NULL && shift_files_cnt > 0) {
    shift_sort_arr = malloc(sizeof(struct sort_t) * shift_files_cnt);
    memset(shift_sort_arr, 0, sizeof(struct sort_t)* shift_files_cnt);
    int i = 0 ;
    for(i = 0; i < shift_files_cnt; i ++) {
  ...
  // 按顺序依次处理每个文件
  for(i = 0; i < files_cnt; i ++) {
    printf ("\nconvert2b.so: %s start at %lu\n", arr[i].filename, arr[i].time);
    char* filename[] = {arr[i].filename};
    convert_one(filename, 1, output_dir, NULL);
  }
